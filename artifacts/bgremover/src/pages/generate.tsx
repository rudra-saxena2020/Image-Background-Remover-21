...
NEVER change white into black, black into white, or any existing color. Never invent stripes, prints, contrast panels, monograms, gradients, metallic finishes, accessories, or decorative elements. Never remove, replace, merge, simplify, reshape, stretch, or redesign any feature. If a detail is unclear, preserve it conservatively and do not invent it.

Generate one clean photograph of exactly one complete product in its normal closed state. Do not create a collage, contact sheet, split screen, extra product, duplicate, prop, packaging, person, hand, mannequin, or floating component. Treat the camera as moving around one real product on a turntable; every output must be the same product with only the requested viewpoint changed.`;
const COMPLETE_STUDIO_SET_PROMPT = `Generate a complete premium e-commerce product photography set using the provided reference images as the exact identity source for one single physical product. If multiple reference images are provided, treat them as multiple views of the same product and use them together to verify shape, color, construction, and attached components. Each requested output must be one separate image containing exactly one photograph of exactly one complete product from exactly one camera angle. The camera may move around the product, but the product itself must not be restyled, reposed, rotated into a new pose, or physically rearranged. Change only camera viewpoint and framing; keep the same studio setup and normal product configuration. Ignore reference backgrounds, layouts, thumbnails, packaging, props, and unrelated objects.

PRODUCT IDENTITY LOCK: The reference product is the absolute source of truth. Preserve its exact shape, silhouette, proportions, dimensions, geometry, material, texture, color, exact color distribution, color calibration, panels, patterns, artwork, stitching, seams, piping, edges, construction, logos, branding, graphics, text placement, hardware, buckles, rings, chains, zippers, metal finish, handles, straps, attachment points, pockets, panels, closures, and exterior details. Every attached component is geometry that must remain fixed: preserve the exact type, count, chain or strap length, thickness, shape, attachment points, hardware, orientation, resting position, physical relationship and visible drape. Do not lengthen, shorten, loosen, tighten, swing, drape, coil, cross, move, disconnect, duplicate, replace, or invent a chain, strap, handle, charm, buckle, ring, or accessory. If a component is not clearly visible in any valid identity reference, do not add it. Do not redesign, reinterpret, simplify, beautify, invent, replace, remove, duplicate, merge, distort, recolor, tint, brighten, darken, or modify any feature.
...
export function Generate() {
  ...
  const handleGenerate = async () => {
    ...
    const singleViewPrompt = trimmed
      ...
      .join("\n")
      .trim();
     // The selected references are multiple views of the same product. Send all
     // supported views so Qwen can cross-check geometry and color instead of
     // reconstructing unseen details from a single image.
